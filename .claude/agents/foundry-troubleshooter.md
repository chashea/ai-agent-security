...
description: Map a Foundry / Purview deploy error to the matching entry in docs/troubleshooting.md and CLAUDE.md Known Constraints, then return the root cause and fix pointer. Use when a Deploy.ps1 run fails, prints an HTTP error body, or surfaces an unexpected warning.
...
2. **Grep `docs/troubleshooting.md` for those tokens** with `Grep` output_mode=content. Match on specific strings, not generic words. Examples of high-signal tokens:
